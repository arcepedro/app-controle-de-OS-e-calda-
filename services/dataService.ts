...
export const DataService = {
  ...
  deleteOperation: (id: string) => {
    const items = getItems<Operation>(STORAGE_KEYS.OPERATIONS);
    setItems(STORAGE_KEYS.OPERATIONS, items.filter(i => i.id !== id));
  },

  deleteOperationsByOS: (osCode: string) => {
    const items = getItems<Operation>(STORAGE_KEYS.OPERATIONS);
    setItems(STORAGE_KEYS.OPERATIONS, items.filter(i => i.osCode !== osCode));
  }
};