...
export const OperationsModule: React.FC = () => {
  ...
  const handleDeleteGroup = (osCode: string) => {
    if (window.confirm(`Tem certeza que deseja excluir todas as operações da O.S. ${osCode}?`)) {
      DataService.deleteOperationsByOS(osCode);
      refreshData();
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(true);
    try {
      const msg = await ExcelParser.readOperations(file);
      refreshData();
      alert(msg);
    } catch (err) {
      console.error(err);
                     ...
                     <table className="min-w-full text-xs">
                        ...
                        <tbody className="divide-y divide-metal-100">
                           {currentOsResources.map(res => {
                             ...
                             return (
                               <tr key={res.id} className={`cursor-pointer transition-colors ${isCurrent ? "bg-brand-blue-50/50 border-l-4 border-brand-blue-500" : "hover:bg-metal-50"}`} onClick={() => handleEditOp(res)} title="Clique para editar este produto">
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-metal-900">Operações</h2>
          <p className="text-brand-slate-500 text-sm font-medium">Gerencie e monitore as ordens de serviço.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button 
            ...
            disabled={isLoading}
            className="flex items-center gap-2 bg-white text-metal-700 border border-metal-200 hover:border-brand-blue-500 hover:text-brand-blue-500 font-semibold py-2 px-4 rounded-xl shadow-sm transition-all"
          >
            {isLoading ? <Loader2 size={18} className="animate-spin" /> : <Upload size={18} />}
            <span className="hidden md:inline">Importar Excel</span>
          </button>
          
          <div className="flex items-center gap-2 bg-white border border-metal-200 rounded-xl px-3 py-1.5 shadow-sm">
            <Calendar size={16} className="text-brand-slate-400" />
            <input 
              type="date" 
              value={reportDate}
              onChange={(e) => setReportDate(e.target.value)}
              className="text-xs font-bold text-metal-700 outline-none bg-transparent"
            />
            <button 
              onClick={handleGeneratePDF}
              className="ml-2 p-1.5 bg-brand-blue-50 text-brand-blue-600 hover:bg-brand-blue-100 rounded-lg transition-colors"
              title="Gerar PDF Diário"
            >
              <FileText size={18} />
            </button>
          </div>
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-brand-blue-500 to-brand-blue-900 hover:to-brand-blue-500 text-white font-bold py-2 px-4 rounded-xl shadow-lg transition-all"
          >
            <Plus size={20} />
